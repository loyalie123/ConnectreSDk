...
class OtpActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        phoneNumber = intent.getStringExtra(PHN_NUM)!!
        ccode=intent.getStringExtra(CCODE)!!
        email=intent.getStringExtra(EMAIL)!!
        if(email!=""){
            verifyLabelTV.text="Verify Your Email"
            enterOtpLabelTV.text = "${getString(R.string.enter_the_otp_send_to)} ${email}"
        }
        else{
            verifyLabelTV.text="Verify Your Mobile"
            enterOtpLabelTV.text = "${getString(R.string.enter_the_otp_send_to)+" +"} ${ccode}${phoneNumber}"
        }
        setupOTPFields()
        ...
        resendOTPTV.setOnClickListener {
            viewModel.resendOtp(phoneNumber, isLogin,ccode,email)
        }
    }

    private fun observeVM() {
        viewModel.otpVerifyHolder.observe(this, Observer {
            when (it) {
                is ViewState.Success -> {
                    loadingDialog.remove()
//                    preferenceStorage.userCCode
                    if (isLogin) {

                        if (preferenceStorage.regDone == 0) {
                        ...
                        } else if (preferenceStorage.regDone == 1 && preferenceStorage.verifyEmail == 0 && preferenceStorage.userType == 2) {

                            preferenceStorage.loginStatus = false
                            OtpEmailActivity.start(this, false, preferenceStorage.userEmail)
                            finish()
                            /*    startActivity(Intent(this, OtpEmailActivity::class.java).putExtra().apply {
                                    flags = (Intent.FLAG_ACTIVITY_CLEAR_TOP
                                            or Intent.FLAG_ACTIVITY_CLEAR_TASK
                                            or Intent.FLAG_ACTIVITY_SINGLE_TOP
                                            or Intent.FLAG_ACTIVITY_NEW_TASK)
                        ...
                        } else if (preferenceStorage.regDone == 1 && preferenceStorage.verifyEmail == 0 && preferenceStorage.userType == 1) {
                            preferenceStorage.loginStatus = true
                            viewModel.getDevelopers()
                        } else if (preferenceStorage.regDone == 1 && preferenceStorage.verifyEmail == 1) {
                            preferenceStorage.loginStatus = true
                            viewModel.getDevelopers()
                        }
                    } else {
...
//                        developerAdapter?.notifyDataSetChanged()
        ...
        viewModel.resendHolder.observe(this, Observer {
            when (it) {
                is ViewState.Success -> {
                    loadingDialog.remove()
                    it.data.toast(this)
    ...
    companion object {
        ...
        const val PHN_NUM = "phn_num"
        const val EMAIL = "email"
        const val CCODE = "c_code"
        const val REQUEST_CODE = 11
        const val RESULT_CODE = 12
        fun start(context: Activity, isLogin: Boolean, phn: String, email: String?, ccode: String?) {
            context.startActivityForResult(Intent(context, OtpActivity::class.java).apply {
                putExtra(IS_LOGIN, isLogin)
                putExtra(PHN_NUM, phn)
                putExtra(EMAIL,email)
                putExtra(CCODE,ccode)
            }, REQUEST_CODE)
        }
    }
}

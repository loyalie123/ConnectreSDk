...
public class InkPageIndicator extends View implements ViewPager.OnPageChangeListener,
    ...
    // defaults
    private static final int DEFAULT_DOT_SIZE = 6;                      // dp
    private static final int DEFAULT_GAP = 8;                          // dp
    private static final int DEFAULT_ANIM_DURATION = 300;               // ms
...
//                attrs, R.styleable.InkPageIndicator, defStyle, 0);
        ...
        selectedColour = ConnectReApp.getThemeColor();

        // unselectedColour = ColorUtils.setAlphaComponent(selectedColour, 200);
        float[] hsv = new float[3];
        Color.colorToHSV(selectedColour, hsv);
        hsv[2] *= 1.5f; // value component
        unselectedColour = ColorUtils.setAlphaComponent(Color.HSVToColor(hsv), 122);
    ...
    private ValueAnimator createMoveSelectedAnimator(
        ...
        moveSelected.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                selectedDotX = (Float) animation.getAnimatedValue();
        ...
        public PendingRetreatAnimator(int was, int now, int steps, StartPredicate predicate) {
            ...
            if (initialX1 != finalX1) { // rightward retreat
                ...
                addUpdateListener(new AnimatorUpdateListener() {
                                      @Override
                                      public void onAnimationUpdate(ValueAnimator animation) {
                                          retreatingJoinX1 = (Float) animation.getAnimatedValue();
                                          postInvalidateOnAnimation();
                ...
                addUpdateListener(new AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(ValueAnimator animation) {
                        retreatingJoinX2 = (Float) animation.getAnimatedValue();
                        postInvalidateOnAnimation();
                        // start any reveal animations if we've passed them
                        for (PendingRevealAnimator pendingReveal : revealAnimations) {
